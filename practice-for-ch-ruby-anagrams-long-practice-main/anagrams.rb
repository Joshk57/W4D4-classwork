...
def second_anagram?(str1, str2)
	arr1 = str1.split("") # O(1) + O(n)
	arr2 = str2.split("") # O(1) + O(n)
	arr1.each_with_index do |char, idx1| # O(n)
		idx2 = arr2.index(char) # O(1) + O(n)
		# debugger
		(idx2.nil?) ? (return false) : arr2.delete_at(idx2) # O(1) + O(1) + O(1)
	end
	arr2.empty? # O(1)
end
